        ...
        try:
            ...
            for batch in parquet_file.iter_batches(batch_size=1000):
                ...
                for _, row in df.iterrows():
                    ...
                    # Enforce all REQUIRED_FIELDS are present and not null.
                    
                    # if any(k not in metadata or pd.isna(metadata[k]) for k in REQUIRED_FIELDS):
                    #     continue  
                     
                    # Skip incomplete log rows!
                    # Carefully order page_content to keep key fields first for better retrieval.
...
def create_vector_db_ivfflat(
    ...
    if not documents:
        ...
        return None
    vector_db_dir = Path(vector_db_dir)  # Should be a directory!
    ...
    print(f"FAISS IndexIVFFlat: training on {embeddings.shape} x {dim}, nlist={nlist}")
    quantizer = faiss.IndexFlatL2(dim)
    ivf_index = faiss.IndexIVFFlat(quantizer, dim, nlist, faiss.METRIC_L2)
    assert not ivf_index.is_trained
    ivf_index.train(embeddings) # type: ignore
    assert ivf_index.is_trained
    ivf_index.add(embeddings) # type: ignore
    ...
    return {
        ...
        "faiss_nlist": nlist,
        "faiss_vectors_added": ivf_index.ntotal,
    }
