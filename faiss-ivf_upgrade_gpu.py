...
def main():
    print("\n--- LANGCHAIN FAISS IVF UPGRADE & (OPTIONAL) MERGE (w/ GPU support) ---\n")
    ...
    print(f"Training IVF index (nlist={nlist}) using GPU ...")
    gpu_res = faiss.StandardGpuResources()
    quantizer = faiss.IndexFlatL2(dim)
    gpu_quantizer = faiss.index_cpu_to_gpu(gpu_res, 0, quantizer)
    gpu_ivf = faiss.IndexIVFFlat(gpu_quantizer, dim, nlist)
    if not gpu_ivf.is_trained:
        gpu_ivf.train(embeddings)
    gpu_ivf.add(embeddings)
    print(f"Converting GPU IVF index to CPU for saving ...")
    cpu_ivf = faiss.index_gpu_to_cpu(gpu_ivf)
    ...
    lc_store.index = cpu_ivf  # monkeypatch in new IVF
    ...
    print(f"IVF index summary: nlist={cpu_ivf.nlist}, ntotal={cpu_ivf.ntotal}, is_trained={cpu_ivf.is_trained}")
